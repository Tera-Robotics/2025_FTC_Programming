...
public final class PinpointLocalizer implements Localizer {
    ...
    public PinpointLocalizer(HardwareMap hardwareMap, double inPerTick, Pose2d initialPose) {
        ...
        double ticksPerRev = 723.24;
        double wheelCircumferenceMM = 48 * Math.PI;
        double ticksPerMM = ticksPerRev / wheelCircumferenceMM;

        driver.setEncoderResolution(ticksPerMM, DistanceUnit.MM);

        double mmPerTick = inPerTick * 25.4;
        driver.setOffsets(
                PARAMS.parYTicks * mmPerTick,
                PARAMS.perpXTicks * mmPerTick,
                DistanceUnit.MM
        );
